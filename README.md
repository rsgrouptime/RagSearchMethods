...
This report provides a comparative analysis of eight Retrieval-Augmented Generation (RAG) search techniques, focusing on their average latency and response times across seven questions submitted to the LLM. Each technique was assessed by averaging the latencies for three main stages—Context Retrieval, Re-ranking, and Generation—and then computing the total response time. Please note that these metrics depend on the specific LLM and embedding models used, response and latency times may vary accordingly.
...
|5	 |Hybrid + HyDE (Hypothetical Document Embedding)|0.11|1.34|2.3|6.5|
|6	 |Hybrid + Hierarchical Indexing|0.3|0.23|0.28|3.28|
|7	 |Hybrid + Parent Document Retriever|0.37|0.18|2.8|6|
|8	 |Hybrid + Sentence Window Retrieval|0.14|0.33|3.78|6.5|
...
3. Higher Generation Latency: Hybrid + Sentence Window Retrieval and Dense Search Fast Embed Model methods exhibited elevated generation latencies, resulting in longer overall response times (6.5 and 6.09 seconds, respectively). These methods might be preferable when context precision is a priority over response speed.
...
**Conclusion:**<br>
Each RAG retrieval method exhibits unique strengths across different latency stages, indicating potential use-case-specific applicability. Hybrid + Hierarchical Indexing is optimal for scenarios requiring minimal response time, while Hybrid approaches with dense, sparse, or hypothetical embedding models may be better suited for complex document handling, even with slightly higher latencies.
This analysis underscores that these response and latency metrics are also influenced by the specific LLM and embedding models employed, allowing for flexibility in achieving optimized outcomes based on system architecture and application needs.
